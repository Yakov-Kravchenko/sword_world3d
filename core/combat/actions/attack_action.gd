...
static func _apply_damage(state: CombatState, actor: CombatActor, target: CombatActor,
	...
	# Отладочный урон — только у своих: если его получают и враги, первый же
	# скелет снимает героя, и режим становится непригоден для проверок.
	if state.dev_damage > 0 and actor.is_hero:
