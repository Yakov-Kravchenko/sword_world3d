...
extends Node3D
## Круг под ногами существа. Синий — персонаж игрока, красный — противник,
## зелёный — герой, который сейчас ходит. Зелёный загорается только у своих:
## он читается как «мой ход», и под врагом сбивал бы с толку. Круг лежит на полу
## чуть выше него, чтобы не мерцать с плитой.

const PARTY := Color(0.36, 0.62, 0.95)
const ENEMY := Color(0.88, 0.30, 0.28)
